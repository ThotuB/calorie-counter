...
impl ProgressDto {
    ...
    pub fn new(meals: Vec<MealGroup>, averages_per_meal_type: Vec<MealAveragePerMealType>) -> Self {
        ProgressDto {
            average: ProgressDto::get_avg_per_day(&meals),
            max: ProgressDto::get_max_per_day(&meals),
            percent_breakfast: ProgressDto::get_percent_meal_type(
                &averages_per_meal_type,
                MealType::Breakfast,
            ),
            percent_lunch: ProgressDto::get_percent_meal_type(
                &averages_per_meal_type,
                MealType::Lunch,
            ),
            percent_dinner: ProgressDto::get_percent_meal_type(
                &averages_per_meal_type,
                MealType::Dinner,
            ),
            percent_snack: ProgressDto::get_percent_meal_type(
                &averages_per_meal_type,
                MealType::Snack,
            ),
    ...
    fn get_avg_per_day(meals: &Vec<MealGroup>) -> ProgressTrackedDto {
        let total_calories: i32 = meals.iter().map(|m| m.calories).sum();
        let total_protein: f32 = meals.iter().map(|m| m.protein).sum();
        let total_carbs: f32 = meals.iter().map(|m| m.carbs).sum();
        let total_fat: f32 = meals.iter().map(|m| m.fat).sum();
    ...
    fn get_max_per_day(meals: &Vec<MealGroup>) -> ProgressTrackedDto {
        let max_calories = meals.iter().map(|m| m.calories).max().unwrap_or(0);
        let max_protein = meals.iter().map(|m| m.protein as i32).max().unwrap_or(0);
        let max_carbs = meals.iter().map(|m| m.carbs as i32).max().unwrap_or(0);
        let max_fat = meals.iter().map(|m| m.fat as i32).max().unwrap_or(0);
    ...
    fn get_percent_meal_type(meal_types: &Vec<MealAveragePerMealType>, meal_type: MealType) -> f32 {
        let total_calories: i32 = meal_types.iter().map(|mt| mt.calories).sum();

        match meal_types.iter().find(|mt| mt.meal_type == meal_type) {
            Some(mt) => mt.calories as f32 / total_calories as f32,
            None => 0.0,
        }
    }
}
...
impl From<MealGroup> for ProgressDayDto {
    fn from(meal: MealGroup) -> Self {
        ProgressDayDto {
            ...
            carbs: meal.carbs as i32,
            protein: meal.protein as i32,
            fat: meal.fat as i32,
            water: 0,
        }
    }
