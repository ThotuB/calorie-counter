...
import LoadingDots from 'src/components/util/LoadingDots';

const EmailSignUp = () => {
	const { createAccountWithEmail, verifyEmail } = useSetup()
	...
	const { mutate: signUpMutation, isLoading: isLoadingSignUp } = useMutation(
		() => createAccountWithEmail(displayName, email, password), {
		onSuccess: () => {
	...
	const { mutate: verifyMutation, isLoading: isLoadingVerify } = useMutation(
		() => verifyEmail(code), {
		onSuccess: () => {
