...
const ProgressDetails: React.FC<{
    stats: StatsDto
}> = ({ stats: { nutrients, vitamins, minerals, amino_acids, calories } }) => {
    const vitaminsVisible = Object.values(vitamins).some(v => v > 0);
    const mineralsVisible = Object.values(minerals).some(v => v > 0);
    const aminoAcidsVisible = Object.values(amino_acids).some(v => v > 0);
    ...
    return (
        <>
            ...
            <View className='mb-12'>
                <TableRow name='Energy' value={calories} unit='kcal' bg='light' />
                <View className='h-6' />
